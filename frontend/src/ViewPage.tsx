  ...
  const handleRefreshScores = async () => {
    ...
    try {
      const response = await fetch(
        `${BACKEND_HTTP_URL}/game/${gameId}/refresh-scores`,
        {
          method: "POST",
          credentials: "include",
        },
      ...
      <div className="bloodbath-page__inner">
        ...
        <div className="view-page__summary">
          ...
          <div className="summary-chip">
            <span className="summary-chip__value">
              {formatCurrency(
                stats.leaderboard.reduce((sum, p) => sum + p.spent, 0),
              )}
            </span>
              ...
              <tbody>
                {stats.leaderboard.map((player, index) => (
                  <tr key={player.name}>
                    ...
                    <td className="stat-table__number stat-table__gold">
                      {player.points}
                    </td>
                    <td className="stat-table__number stat-table__ember">
                      {formatCurrency(player.spent)}
                    </td>
                    <td className="stat-table__number">
                    ...
                    <td>
                      <div className="stat-table__teams">
                        {player.teams.map((team) => (
                          <span
                            className="stat-table__team-chip"
                            key={team.name}
                          >
                            {team.name} ({team.seed}) —{" "}
                            {formatCurrency(team.purchasePrice || 0)}
                          </span>
                        ))}
                      </div>
                    </td>
                ...
                {stats.avgCostPerSeed.length === 0 && (
                  <tr>
                    <td
                      ...
                      style={{
                        textAlign: "center",
                        color: "var(--bloodbath-copy-muted)",
                      }}
                    >
                      No sales yet
                    </td>
              ...
              <table className="stat-table">
                ...
                <tbody>
                  {stats.auctionHistory.map((sale, index) => (
                    <tr key={index}>
                      <td
                        className="stat-table__number"
                        style={{ color: "var(--bloodbath-copy-muted)" }}
                      >
                        {index + 1}
                      </td>
