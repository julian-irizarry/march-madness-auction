...
const AVAILABILITY_FILTER_OPTIONS: Array<{
...
function getAuctionRoundKey(
  team?: Pick<TeamInfo, "shortName" | "urlName" | "region" | "seed"> | null,
) {
  if (!team) {
    return "";
  }
...
function countAuctionRounds(teams: TeamInfo[]) {
  return new Set(teams.map((team) => getAuctionRoundKey(team)).filter(Boolean))
    .size;
}

function getTeamDisplayName(
  team: Pick<TeamInfo, "shortName" | "region" | "seed">,
) {
        ...
        {props.players.length > 0 ? (
          props.players.map(([playerName, playerInfo]) => {
            ...
            const isHighestBidder =
              Boolean(props.currentBidder) &&
              props.currentBidder === playerName;
            const isCurrentPlayer =
              Boolean(props.currentPlayerName) &&
              props.currentPlayerName === playerName;
                ...
                <div className="player-rail__card-shell">
                  <div className="player-rail__summary">
                    <div className="player-rail__identity">
                      <div className="player-rail__copy">
                        <div className="player-rail__name-line">
                          <span className="player-rail__name">
                        ...
                        <div className="player-rail__meta-line">
                          <span
                            className={[
                              ...
                              playerInfo.balance < 25
                                ? "player-rail__balance--danger"
                                : "",
                            ]
                              .filter(Boolean)
                              .join(" ")}
                    ...
                    <div className="player-rail__actions">
                      <span className="player-rail__badge">
                        {getTeamCountLabel(teamCount)}
                      </span>
                      {isHighestBidder ? (
                  ...
                  <div className="player-rail__details">
                    <div className="player-rail__details-inner">
                      <div className="player-rail__details-label">
                        Owned Teams
                      </div>
                      {teamCount > 0 ? (
                        playerInfo.teams.map((ownedTeam, teamIndex) => {
                          ...
                          return (
                            <div
                              className="player-rail__team-row"
                              key={`${playerName}_${ownedTeam.shortName}_${teamIndex}`}
                            >
                              <div className="player-rail__team-main">
                                <span
                                  className="player-rail__team-logo-shell"
                                  aria-hidden="true"
                                >
                                  {teamLogoUrl ? (
                                    <img
                                    ...
                                    <span className="player-rail__team-logo-fallback">
                                      {ownedTeam.shortName
                                        .trim()
                                        .charAt(0)
                                        .toUpperCase() || "?"}
                                ...
                                <span className="player-rail__team-copy">
                                  <span className="player-rail__team-name">
                                    {ownedTeam.shortName.toUpperCase()}
                                  </span>
                                  {ownedTeam.seed > 0 ? (
                                    <span className="player-rail__team-seed">
                                      #{ownedTeam.seed}
                                    </span>
                                  ) : null}
                                </span>
                              ...
                              <span className="player-rail__team-price">
                                {formatCurrency(ownedTeam.purchasePrice)}
                              </span>
                            </div>
                          );
                        })
                      ) : (
                        <p className="player-rail__empty-copy">
                          No teams owned yet.
                        </p>
                      )}
                    </div>
          ...
          <p className="player-rail__empty-copy player-rail__empty-copy--panel">
            No players yet.
          </p>
        )}
      </div>
...
function TeamsRail(props: TeamsRailProps) {
    ...
    if (isSeedBundleTeam(props.currentTeam)) {
      return (
        props.entries.find(
          (entry) =>
            entry.status === "live" &&
            entry.bundleSeed === props.currentTeam.seed,
        ) || null
      );
    }
  ...
  const hasMatchingEntries = regionSections.some(
  ...
  return (
    <aside
      ...
      style={
        {
          "--team-rail-sticky-offset": `${stickyOffset}px`,
    ...
    >
      <div className="team-rail__sticky" ref={stickyRef}>
          ...
          {props.currentTeam.shortName ? (
            <button
              ...
              className={[
                ...
                liveSpotlightEntry &&
                props.inspectedTeamKey === liveSpotlightEntry.key
                  ? "team-rail__spotlight--inspected"
                  : "",
              ]
                .filter(Boolean)
                .join(" ")}
            ...
            >
              <div className="team-rail__spotlight-topline">
                ...
                <span className="team-rail__spotlight-region">
                  {getTeamRegionLabel(props.currentTeam)}
                </span>
              </div>

              <div className="team-rail__spotlight-main">
                <span
                  className="team-rail__team-logo-shell team-rail__team-logo-shell--spotlight"
                  aria-hidden="true"
                >
                  {!isSeedBundleTeam(props.currentTeam) &&
                  getTeamLogoUrl(props.currentTeam.urlName) ? (
                    <img
                    ...
                    <span className="team-rail__team-logo-fallback">
                      {isSeedBundleTeam(props.currentTeam)
                ...
                <span className="team-rail__spotlight-copy">
                  <span className="team-rail__spotlight-name">
                    {getTeamDisplayName(props.currentTeam)}
                  </span>
            ...
            <div className="team-rail__spotlight team-rail__spotlight--idle">
              <span className="team-rail__spotlight-tag">On The Block</span>
              <span className="team-rail__spotlight-empty">
                Awaiting the next team.
              </span>
            </div>
          )}
        </div>
        ...
        <div className="team-rail__controls">
          <div
            className="team-rail__filter-row"
            role="tablist"
            ...
            {AVAILABILITY_FILTER_OPTIONS.map((option) => (
              <button
                ...
                className={[
                  ...
                  props.availabilityFilter === option.value
                    ? "team-rail__filter-button--active"
                    : "",
                ]
                  .filter(Boolean)
                  .join(" ")}
              ...
              >
                ...
                <span className="team-rail__filter-count">
      ...
      <div className="team-rail__sections">
        {regionSections.map((section) => {
          ...
          const isExpanded = sectionIsExpanded(
            section.id,
            ...
            <section className="team-rail__section" key={section.id}>
              <button
                ...
                className={[
                  ...
                  isExpanded && !hasSearch
                    ? "team-rail__section-toggle--expanded"
                    : "",
                ]
                  .filter(Boolean)
                  .join(" ")}
              ...
              >
                <span className="team-rail__section-copy">
                  <span className="team-rail__section-title">
                    {section.title}
                  </span>
                  <span className="team-rail__section-count">
                    {section.entries.length}
                  </span>
                </span>

                <span className="team-rail__section-caret">
                  {isExpanded ? "−" : "+"}
                </span>
              </button>
              ...
              {isExpanded ? (
                <div className="team-rail__section-grid">
                  {section.entries.map((entry) => {
                    ...
                    const isExpandedSoldCard =
                      props.expandedSoldTeamKey === entry.key;
                        ...
                        <span className="team-rail__team-card-shell">
                          <span className="team-rail__team-card-topline">
                            <span className="team-rail__team-card-meta">
                              <span className="team-rail__team-seed-tag">
                                #{entry.team.seed}
                              </span>
                            </span>
                          ...
                          <span className="team-rail__team-card-main">
                            <span
                              className="team-rail__team-logo-shell"
                              aria-hidden="true"
                            >
                              {teamLogoUrl ? (
                                <img
                            ...
                            <span className="team-rail__team-card-copy">
                              <span className="team-rail__team-card-name">
                                {entry.displayName}
                              </span>
                            </span>
                          </span>
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if ("players" in data && data.players) {
          ...
          const remaining = data.remaining.map(
            (tempTeam: { [key: string]: any }) => ({
              shortName: tempTeam.shortName,
              urlName: tempTeam.urlName,
              seed: tempTeam.seed,
              region: tempTeam.region,
          ...
          const nextAllTeams = data.all_teams.map(
            (tempTeam: { [key: string]: any }) => ({
              shortName: tempTeam.shortName,
              urlName: tempTeam.urlName,
              seed: tempTeam.seed,
...
function GamePage() {
  ...
  const [expandedPlayerName, setExpandedPlayerName] = useState<string | null>(
    null,
  );
  ...
  const [inspectedTeamTone, setInspectedTeamTone] =
    useState<InspectedTeamTone | null>(null);
  const [inspectedBundleSeed, setInspectedBundleSeed] = useState<number | null>(
    null,
  );
  const [expandedSoldTeamKey, setExpandedSoldTeamKey] = useState<string | null>(
    null,
  );
  ...
  const auctionNumber = useMemo(() => {
    ...
    return Math.min(
      totalAuctions,
      Math.max(1, totalAuctions - remainingAuctionRounds),
    );
  }, [isAuctionLive, remainingTeams, totalAuctions]);
  const activePlayerBalance = playerName
    ? playerInfos.get(playerName)?.balance || 0
    : 0;
  const orderedPlayers = Array.from(playerInfos.entries());
  const expandedPlayerTeams = expandedPlayerName
    ? playerInfos.get(expandedPlayerName)?.teams || []
    : [];
  const highlightedTeamKeys = Array.from(
    new Set(
      expandedPlayerTeams
        .map((ownedTeam) => normalizeTeamKey(ownedTeam))
        .filter(Boolean),
    ),
  );
  ...
  const uniqueTeamRailEntries = useMemo(() => {
    ...
    const remainingTeamKeys = new Set(
      remainingTeams
        .map((remainingTeam) => normalizeTeamKey(remainingTeam))
        .filter(Boolean),
    );
    const ownedTeamMap = new Map<
      string,
      { ownerName: string; purchasePrice?: number; team: TeamInfo }
    >();
    const soldBundleMap = new Map<
      number,
      { ownerName: string; purchasePrice?: number }
    >();
    ...
    allTeams.forEach((availableTeam) => {
      ...
      const soldBundleRecord =
        availableTeam.seed === 15 || availableTeam.seed === 16
          ? soldBundleMap.get(availableTeam.seed)
          : undefined;
      const isLiveTeam = liveTeamKey === teamKey;
      const isLiveBundleTeam =
        liveBundleSeed !== null && availableTeam.seed === liveBundleSeed;

      if (soldBundleRecord) {
        teamRailEntries.push(
          buildTeamRailEntry(availableTeam, "sold", {
      ...
      if (isLiveBundleTeam || isLiveTeam) {
        teamRailEntries.push(
          buildTeamRailEntry(availableTeam, "live", {
      ...
      if (ownedRecord) {
        teamRailEntries.push(
          buildTeamRailEntry(ownedRecord.team, "sold", {
  ...
  useEffect(() => {
    setOpenTeamSections((currentSections) => {
      const validSections = currentSections.filter((sectionId) =>
  ...
  useEffect(() => {
    ...
    const hasVisibleEntry = uniqueTeamRailEntries.some(
      (entry) => entry.key === inspectedKey,
    );
    if (!hasVisibleEntry) {
  ...
  useEffect(() => {
    if (
      expandedSoldTeamKey &&
      !uniqueTeamRailEntries.some(
        (entry) => entry.status === "sold" && entry.key === expandedSoldTeamKey,
      )
    ) {
  ...
  const handleTogglePlayer = (nextPlayerName: string) => {
    setExpandedPlayerName((currentPlayerName) =>
      currentPlayerName === nextPlayerName ? null : nextPlayerName,
    );
  };
  ...
  const handleToggleTeamSection = (sectionId: string) => {
    ...
    setOpenTeamSections((currentSections) =>
      currentSections.includes(sectionId) ? [] : [sectionId],
    );
  };
  ...
  const handleInspectTeam = (entry: TeamRailEntry) => {
    ...
    if (entry.status === "sold" && shouldUseTapSoldCardReveal()) {
      setExpandedSoldTeamKey((currentKey) =>
        currentKey === entry.key ? null : entry.key,
      );
      return;
    }
        ...
        <div className="graveyard-bracket__header game-page__top-rail">
          ...
          <div className="graveyard-bracket__meta">
            ...
            <div className="graveyard-bracket__meta-chip">
              ...
              <button
                type="button"
                className="graveyard-bracket__copy-button"
            ...
            <div
              className={[
                ...
                isAuctionLive
                  ? "graveyard-bracket__meta-chip--live-active"
                  : "graveyard-bracket__meta-chip--live-complete",
              ].join(" ")}
            >
              <span
                className="graveyard-bracket__live-indicator"
                aria-hidden="true"
              />
              <span className="graveyard-bracket__meta-label">
                {isAuctionLive ? "LIVE" : "DONE"}
              </span>
            </div>
          </div>
        </div>
