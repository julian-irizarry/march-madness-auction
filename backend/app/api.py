...
from app import (GAME_ID_NUM_CHAR, BidModel, CreateModel, GameTracker,
                 JoinModel, ViewModel)
from app.types.types import (INITIAL_BALANCE, GamePhase, jsonify_dict,
                             jsonify_list)
...
async def create_game(create_model: CreateModel, response: Response) -> dict:
    new_game_id: str = "".join(
        random.choices(string.ascii_uppercase + string.digits, k=GAME_ID_NUM_CHAR)
    )
    gameTracker.add_game(gameId=new_game_id, creator=create_model.player)
    game_connections[new_game_id] = (
        []
    )  # Initialize the list of WebSocket connections for this game

    # Create session
    session_id = str(uuid.uuid4())
    sessions[session_id] = {
        "gameId": new_game_id,
        "playerName": create_model.player,
        "isCreator": True,
    }
...
async def join_game(join_model: JoinModel, request: Request, response: Response):
    ...
    if join_model.player in gameTracker.get_all_players(join_model.gameId):
        ...
        if session_id and session_id in sessions:
            ...
            if (
                session["gameId"] == join_model.gameId
                and session["playerName"] == join_model.player
            ):
                # Valid rejoin — refresh the cookie and let them back in
                set_session_cookie(response, session_id)
                return {"detail": "Rejoined game successfully"}
        raise HTTPException(
            status_code=400, detail="Player name already taken in this game"
        )
    ...
    session_id = str(uuid.uuid4())
    sessions[session_id] = {
        "gameId": join_model.gameId,
        "playerName": join_model.player,
        "isCreator": False,
    }
    set_session_cookie(response, session_id)
    ...
    if join_model.gameId in game_connections:
        for ws in game_connections[join_model.gameId]:
            await ws.send_json(
                {
                    "players": jsonify_dict(
                        gameTracker.get_all_players(join_model.gameId)
                    )
                }
            )
    ...
    if view_model.gameId in game_connections:
        for ws in game_connections[view_model.gameId]:
            await ws.send_json(
                {
                    "players": jsonify_dict(
                        gameTracker.get_all_players(view_model.gameId)
                    )
                }
            )
...
async def finalize_bid(game_id: str):
    ...
    for ws in game_connections[game_id]:
        ...
        await ws.send_json(
            {"team": current_team.model_dump() if current_team else None}
        )
        ...
        try:
            while True:
                if websocket.application_state == WebSocketState.CONNECTED:
                    ...
                    await websocket.send_json(
                        {"current_bidder": gameTracker.get_current_bidder(game_id)}
                    )
                else:
                    break  # Stop sending if the connection is no longer active
                await asyncio.sleep(10)
        ...
        try:
            while True:
                if websocket.application_state == WebSocketState.CONNECTED:
                    await websocket.send_json(
                        {
                            "remaining": jsonify_list(
                                gameTracker.get_remaining_teams(game_id)
                            )
                        }
                    )
                else:
        ...
        try:
            while True:
                if websocket.application_state == WebSocketState.CONNECTED:
                    ...
                    if (
                        message == "startGame"
                        and websocket in game_connections[game_id][:1]
                    ):
...
async def bid(bid_model: BidModel):
    ...
    if bid_model.gameId in game_connections:
        for ws in game_connections[bid_model.gameId]:
            ...
            await ws.send_json(
                {"current_bidder": gameTracker.get_current_bidder(bid_model.gameId)}
            )
            await ws.send_json(
                {
    ...
    if (
        bid_model.gameId in countdown_tasks
        and not countdown_tasks[bid_model.gameId].cancelled()
    ):
        countdown_tasks[bid_model.gameId].cancel()

    countdown_tasks[bid_model.gameId] = asyncio.create_task(
        start_countdown(bid_model.gameId)
    )
